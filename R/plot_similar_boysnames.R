...
#' It shows trends over years using per million births for similar names for boys.
#'
#' @param input_name2 A name string to find phonetically similar boys' names.
...
plot_similar_boysnames <- function(input_name2, max_distance = 1) {
  filteredb <- irishbabynames %>%
    filter(Statistic == "Boys Names in Ireland with 3 or More Occurrences")

  boys_similar1 <- filteredb %>%
    pivot_longer(
      cols = matches("^[0-9]{4}$"),
      names_to = "Year",
      values_to = "Value"
  ...
  clean_input1 <- stri_trans_general(input_name2, "Latin-ASCII")
  ...
  p1 <- ggplot() +
    ...
    labs(
      title = paste("Similar Names to:", input_name2),
      x = "Year",
